...
if __name__ == '__main__':
    ...
    settings = os.path.join(projectdir, 'settings')
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            settings = os.path.join(projectdir, arg) 
            
    if not os.path.isfile('%s.py' % settings):
        sys.stderr.write('The settings file does not exist: %s.py' % settings)
        usage()
        sys.exit(2)
